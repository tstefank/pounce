...
- **Correlator:** joins intent ↔ OS events by **PID + time window**; flags divergence (discrepancy = the signal). (Phase 3.) Built on network events first; `eslogger` file/exec events slot in later through the same generic OS-event interface — so write the join against the event type, not against network specifically.
