...
// localTest treats 192.168.1.16 (the capture host) and loopback as local.
func localTest(ip string) bool {
	switch ip {
	case "192.168.1.16", "127.0.0.1", "::1":
		return true
	}
	return false
}
...
func TestParseLine_RealPktapSamples(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "empty metadata () -> not attributable",
...
func TestParseLine_DedupKeyStableAcrossPackets(t *testing.T) {
	src := &PktapSource{IsLocal: localTest}
	// Two packets of the same connection must produce the same dedup key.
	...
	_, k1, ok1 := src.parseLine(l1)
	_, k2, ok2 := src.parseLine(l2)
	if !ok1 || !ok2 {
		t.Fatal("both lines should parse")
	}
	if k1 != k2 {
